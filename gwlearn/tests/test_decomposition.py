"""
Tests for gwlearn.decomposition: GWPCA and RobustGWPCA.
...
from gwlearn.decomposition import GWFA, GWPCA, RobustGWPCA
...
class TestGWPCAFit:
    ...
    def test_fit_all_components(self, sample_decomposition_data):
        ...
        X, geometry = sample_decomposition_data
        n_features = X.shape[1]
        model = GWPCA(n_components=None, bandwidth=SMALL_BW).fit(
            X, geometry=geometry
        )
    ...
    def test_names_match_index(self, sample_decomposition_data):
        X, geometry = sample_decomposition_data
        model = GWPCA(n_components=2, bandwidth=SMALL_BW).fit(
            X, geometry=geometry
        )
...
class TestGWPCAInvariants:
    ...
    def test_eigenvalues_descending(self, sample_decomposition_data):
        ...
        for i in range(len(model._names)):
            ...
            assert np.all(np.diff(eigs) <= 1e-10), f"Eigenvalues not descending at loc {i}"
...
class TestGWPCADerivedAttributes:
    ...
    def test_global_model_fitted(self, sample_decomposition_data):
        X, geometry = sample_decomposition_data
        model = GWPCA(n_components=N_COMP, bandwidth=SMALL_BW, fit_global_model=True).fit(
            X, geometry=geometry
        )
...
class TestRobustGWPCA:
    def test_fit_shapes(self, sample_decomposition_data):
        X, geometry = sample_decomposition_data
        model = RobustGWPCA(n_components=N_COMP, bandwidth=SMALL_BW).fit(
            X, geometry=geometry
        )
        assert model.components_.shape == (N_OBS, X.shape[1], N_COMP)
        assert model.explained_variance_.shape == (N_OBS, N_COMP)
        assert model.scores_.shape == (N_OBS, N_COMP)

    def test_outlier_distances_shape(self, sample_decomposition_data):
        X, geometry = sample_decomposition_data
        model = RobustGWPCA(n_components=N_COMP, bandwidth=SMALL_BW).fit(
            X, geometry=geometry
        )
        sd, od = model.outlier_distances(X)
        assert len(sd) == N_OBS
        assert len(od) == N_OBS

    def test_outlier_distances_nonneg(self, sample_decomposition_data):
        X, geometry = sample_decomposition_data
        model = RobustGWPCA(n_components=N_COMP, bandwidth=SMALL_BW).fit(
            X, geometry=geometry
        )
    ...
    def test_classify_outliers_categories(self, sample_decomposition_data):
        X, geometry = sample_decomposition_data
        model = RobustGWPCA(n_components=N_COMP, bandwidth=SMALL_BW).fit(
            X, geometry=geometry
        )
        cats = model.classify_outliers(X)
        assert set(cats.unique()).issubset({0, 1, 2, 3})
        assert len(cats) == N_OBS

    def test_support_fraction_parameter(self, sample_decomposition_data):
        X, geometry = sample_decomposition_data
        model = RobustGWPCA(
            n_components=2, bandwidth=SMALL_BW, support_fraction=0.75
        ).fit(X, geometry=geometry)
        assert model.support_fraction == 0.75
        assert model.components_.shape[0] == N_OBS


# ---------------------------------------------------------------------------
# GWPCA — Monte Carlo stationarity test
# ---------------------------------------------------------------------------


class TestStationarityTest:
    def test_stationarity_test_returns_dict(self, sample_decomposition_data):
        X, geometry = sample_decomposition_data
        model = GWPCA(n_components=N_COMP, bandwidth=SMALL_BW).fit(
            X, geometry=geometry
        )
        result = model.stationarity_test(
            X, geometry, component=0, n_permutations=9, random_state=42
        )
        assert "true_sd" in result
        assert "permuted_sds" in result
        assert "p_value" in result
        assert 0.0 <= result["p_value"] <= 1.0
        assert len(result["permuted_sds"]) == 9

    def test_true_sd_is_positive(self, sample_decomposition_data):
        X, geometry = sample_decomposition_data
        model = GWPCA(n_components=N_COMP, bandwidth=SMALL_BW).fit(
            X, geometry=geometry
        )
        result = model.stationarity_test(
            X, geometry, component=0, n_permutations=5, random_state=0
        )
...
class TestGWFAFit:
    def test_fit_returns_self(self, sample_decomposition_data):
        X, geometry = sample_decomposition_data
        model = GWFA(n_components=2, bandwidth=SMALL_BW)
        result = model.fit(X, geometry=geometry)
        assert result is model
    ...
    def test_fit_shapes(self, sample_decomposition_data):
        ...
        n_features = X.shape[1]
        q = 2
        model = GWFA(n_components=q, bandwidth=SMALL_BW).fit(X, geometry=geometry)
        assert model.components_.shape == (N_OBS, n_features, q)
        assert model.explained_variance_.shape == (N_OBS, q)
        assert model.scores_.shape == (N_OBS, q)
        assert model.local_means_.shape == (N_OBS, n_features)

    def test_noise_variance_shape(self, sample_decomposition_data):
        X, geometry = sample_decomposition_data
        model = GWFA(n_components=2, bandwidth=SMALL_BW).fit(X, geometry=geometry)
        assert model.noise_variance_.shape == (N_OBS, X.shape[1])

    def test_communalities_shape(self, sample_decomposition_data):
        X, geometry = sample_decomposition_data
        model = GWFA(n_components=2, bandwidth=SMALL_BW).fit(X, geometry=geometry)
        assert model.communalities_.shape == (N_OBS, X.shape[1])
    ...
    def test_adaptive_bandwidth(self, sample_decomposition_data):
        X, geometry = sample_decomposition_data
        model = GWFA(n_components=2, bandwidth=SMALL_BW, fixed=False).fit(
            X, geometry=geometry
        )
        assert model.components_.shape[0] == N_OBS

    def test_fixed_bandwidth(self, sample_decomposition_data):
        X, geometry = sample_decomposition_data
        model = GWFA(n_components=2, bandwidth=200_000, fixed=True).fit(
            X, geometry=geometry
        )
        assert model.components_.shape[0] == N_OBS


class TestGWFAInvariants:
    def test_factor_variance_nonneg(self, sample_decomposition_data):
        """Factor variance (sum of squared loadings) must be ≥ 0."""
        X, geometry = sample_decomposition_data
        model = GWFA(n_components=2, bandwidth=SMALL_BW).fit(X, geometry=geometry)
        assert np.all(model.explained_variance_ >= -1e-10)

    def test_communalities_nonneg(self, sample_decomposition_data):
        X, geometry = sample_decomposition_data
        model = GWFA(n_components=2, bandwidth=SMALL_BW).fit(X, geometry=geometry)
        assert np.all(model.communalities_ >= -1e-10)

    def test_noise_variance_nonneg(self, sample_decomposition_data):
        """Unique variances Ψ must be ≥ 0."""
        X, geometry = sample_decomposition_data
        model = GWFA(n_components=2, bandwidth=SMALL_BW).fit(X, geometry=geometry)
        # Allow NaN for fallback locations; check non-NaN values only
        valid = model.noise_variance_[~np.isnan(model.noise_variance_)]
        assert np.all(valid >= -1e-10)

    def test_explained_variance_ratio_in_01(self, sample_decomposition_data):
        X, geometry = sample_decomposition_data
        model = GWFA(n_components=2, bandwidth=SMALL_BW).fit(X, geometry=geometry)
...
class TestGWFAAttributes:
    def test_global_model_is_factor_analysis(self, sample_decomposition_data):
        from sklearn.decomposition import FactorAnalysis

        X, geometry = sample_decomposition_data
        model = GWFA(n_components=2, bandwidth=SMALL_BW, fit_global_model=True).fit(
            X, geometry=geometry
        )
        assert hasattr(model, "global_model")
        assert isinstance(model.global_model, FactorAnalysis)

    def test_no_global_model_when_disabled(self, sample_decomposition_data):
        X, geometry = sample_decomposition_data
        model = GWFA(
            n_components=2, bandwidth=SMALL_BW, fit_global_model=False
        ).fit(X, geometry=geometry)
        assert not hasattr(model, "global_model")

    def test_winning_variable(self, sample_decomposition_data):
        X, geometry = sample_decomposition_data
        model = GWFA(n_components=2, bandwidth=SMALL_BW).fit(X, geometry=geometry)
        wv = model.winning_variable_
        assert isinstance(wv, pd.Series)
    ...
    def test_rotation_varimax(self, sample_decomposition_data):
        """Varimax rotation is accepted and changes the loadings."""
        X, geometry = sample_decomposition_data
        m_none = GWFA(n_components=2, bandwidth=SMALL_BW, rotation=None).fit(
            X, geometry=geometry
        )
        m_varimax = GWFA(n_components=2, bandwidth=SMALL_BW, rotation="varimax").fit(
            X, geometry=geometry
        )
        assert m_varimax.components_.shape == m_none.components_.shape
        # Loadings differ after rotation
        assert not np.allclose(
            np.abs(m_varimax.components_), np.abs(m_none.components_), atol=1e-6
        )


class TestGWFASklearnAPI:
    def test_get_params(self, sample_decomposition_data):
        model = GWFA(n_components=2, bandwidth=SMALL_BW, rotation="varimax")
        params = model.get_params()
        assert params["n_components"] == 2
        assert params["bandwidth"] == SMALL_BW
        assert params["rotation"] == "varimax"
