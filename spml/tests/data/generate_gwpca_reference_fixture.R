# Script to generate an external reference fixture for pytest.
# Run this on a machine with the 'GWmodel' and 'jsonlite' packages installed.
# It will generate 'gwpca_reference_fixture.json' for Python-side comparison tests.
...
# GWPCA(n_components=2, bandwidth=5, fixed=True, kernel="bisquare",
#       include_focal=True)
model <- gwpca(sdf, vars=c("A", "B", "C"), bw=5, k=2, kernel="bisquare", adaptive=FALSE, scaling=FALSE)

# 3. Extract the outputs
# loadings: a 10 x 3 x 2 array
loadings <- model$loadings

# var: local variance
variance <- model$var

# explained variance ratio (proportion) -> the fixture stores percentages
